...
class HeapManager:
  ...
  def deallocate(self,address) :
    ...
    pointer = self.freeStart
    while pointer != NULL and pointer < currentAddress:
      lag = pointer
      pointer = self.memory[ pointer + 1 ]
    if currentAddress + self.memory [currentAddress ] == pointer:
      self.memory[currentAddress] += self.memory[ pointer ]
      pointer = self.memory[ pointer + 1 ]
    if lag == NULL:
      self.freeStart = currentAddress
      self.memory [currentAddress + 1 ] = pointer
    elif lag + self.memory[lag] == currentAddress:
      self.memory[ lag ] += self.memory[ currentAddress ]
      self.memory[ lag + 1 ] = pointer
    else:
      self.memory[ lag + 1 ] = currentAddress
      self.memory[ currentAddress + 1 ] = pointer
