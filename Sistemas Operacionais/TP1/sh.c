...
/* MARK NAME Lucca Silva Medeiros */
/* MARK NAME Gabriel Sacoman Teixeira Silva */
...
runcmd(struct cmd *cmd)
{
  ...
  case '<':
    ...
    int fileValue = open(rcmd -> file, rcmd -> mode );

    dup2( fileValue, rcmd -> fd);
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
     ...
     * para reportar o erro corretamente. 
     ...
     O IF abaixo verifica se o comando atual é o cd.
     Esse comando permite ao usuário mudar o diretório de trabalho,
     por isso o IF é necessário, pois ele faz com que o diretório de 
     trabalho mude para a execução do processo. Além disso também
     possui chamadas de erros específicos de mudança de diretório
     caso não ocorra a mudança corretamente.
     
     */
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      buf[strlen(buf)-1] = 0;
      if(chdir(buf+3) < 0)
        fprintf(stderr, "Erro na mudança de diretório.\n");
      continue;
    }
    /* MARK END task1 */
